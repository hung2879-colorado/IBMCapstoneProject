...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                options=[
                                    ...
                                    searchable=True,
                                    style={'width': '80%', 'padding': '10px', 'font-size': '20px'}
                                    ),
                                html.Br(),
...
def get_pie_chart(selected_site):
    if selected_site == 'ALL':
        fig = px.pie(spacex_df, , values = 'class', names='success-pie-chart', title='Total Success Launches')
    else:
        filtered_df = spacex_df[spacex_df['Launch Site'] == selected_site]
        fig = px.pie(filtered_df, values='class', title=f'Success/Failure Launches at {selected_site}')
    return fig
# TASK 4:
...
def get_scatter_chart(selected_site, payload_range):
    ...
    fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class', color='class',
                     ...
                     range_x=[min_payload, max_payload])
    return fig
# Run the app
